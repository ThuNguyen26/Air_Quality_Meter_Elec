...
#include <stdint.h>
#include "Pzem004T.h"
void Pzem_Init(Pzem *p, HardwareSerial *port, uint8_t rx_pin, uint8_t tx_pin, uint8_t addr)
{
  port->begin(9600, SERIAL_8N1, rx_pin, tx_pin);
...
uint16_t receive(Pzem *p, uint8_t *resp, uint16_t len)
{
  ...
  {
    if (p->_serial->available() > 0)
    {
      uint8_t c = (uint8_t)p->_serial->read();
      resp[index++] = c;
    }
  }
...
bool sendCmd8(Pzem *p, uint8_t cmd, uint16_t rAddr, uint16_t val, bool check, uint16_t slave_addr)
{
  ...
  p->_serial->write(sendBuffer, 8); // send frame
...
ERROR_CODE updateValues(Pzem *p)
{
  ...
  if (p->_lastRead + 200 > millis())
  {
    return ERROR_NONE;
  }
  sendCmd8(p, 0x04, 0x00, 0x0A, false);

  if (receive(p, response, 25) != 25)
  { // Something went wrong
    return PZEM_GET_DATA_FAIL;
  }
  p->resp = response;
